...
simpleQIF <- function (y, X, id, family = binomial) {
    ...
    beta0 <- fit0$coef
    ## optim step
    res <- optim(beta0, fn = qNbeta, gr = NULL,
                 clusSize = clusSize, clusN = clusN, y = y, X = X,
                 matMList = matMList, famFun = famFun)
...
g_riBeta <- function (matM, betaVec, yi, Xi, famFun) {
    ...
    matDi <- famFun$mu.eta(muiVec) * Xi # element-wise
...
by `simpleQIF` and `geese` are quite close to each other.
So we have confidence that the implementation is correct.
