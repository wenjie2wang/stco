---
title: "STAT-6494 Advanced Statistical Computing with R -- HW 3"
author: Wenjie Wang
...
needpack <- c("ggplot2", "knitr", "Matrix", "microbenchmark")
ipak(needpack)
...
`X` to fit linear regression model. It takes response vector `Y` and design
matrix `X`, which can be sparse matrices object in `dgCMatrix` class
produced by pacakge **Matrix**.
And it returns `coefficients` for estimated
coefficients, `stderr` for standard error of the estimates, and `df.residuals`
for the degrees of freedom of the residuals. 
The function `lmChol` is similarly designed but using Cholesky decomposition.

The straightforward implementation is done completely in R.
Note that for `lmQr` and `lmChol`, the design matrix `X` input can be
either dense matrix class object or sparse matrix class object, such as
`dgCMatrix` class object. The corresponding method will be called depending
on the object class.
...
dimension and sparsity. Then we compare the performance between `lm.fit`
and `lmQr` on those datasets.
...
From the comparison, we find that `lmChol` is the fastest one.
...
ggplot2::autoplot(microbenchmark(lm.fit(X, y), lmQr(X, y), lmQr(Xsp, y),
                                 lmChol(X, y), lmChol(Xsp, y), times = 10))
```

From the comparison, we find that `lmChol` on dense matrix objects is still the
...
Similarly, the performance comparison is shown as follows:
...
From the comparison, we find that `lmChol` on sparse matrix objects becomes the
fastest one. `lmChol` becomes slower when a dense matrix object is specified.
`lm.fit` is slower than `lmChol` but faster than `lmQr`.
One interesting finding is that `lmQr` runs faster when a sparse matrix object
is input instead of a dense matrix object, which is reasonable since
the dimension of the sparse design matrix is much higher than the previous case.
The benefit of using sparse matrix class object to deal with sparse matrices
become more obvious when dimension and sparsity goes larger.
