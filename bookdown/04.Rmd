...
(ref:plotDf5-cap) Sample Spearman's rho (blue solid curve) and reference line
with slope 1 and intercept 0 (black dotted line).

```{r plotDf5, fig.cap = "(ref:plotDf5-cap)"}
library(ggplot2)
...
From Figure \@ref(fig:plotDf5), We may observe that the sample Spearman's rho
curve looks quite linear. Therefore, penalized smoothing cubic spline is
probably a good choice since it penalizes the second derivatives. The tuning
parameter is left to be selected by GCV.
...
The fitted spline function is plotted in Figure \@ref(fig:spline1), from
which we may find that the penalized smoothing cubic spline fits the sample
Spearman's rho quite well.
...
(ref:plotDf10-cap) Sample Spearman's rho (red solid curve) and reference line
with slope 1 and intercept 0 (black dotted line).

```{r plotDf10, fig.cap = "(ref:plotDf10-cap)"}
...
(ref:spline2) Fitted Spearman's rho curve by penalized spline.

```{r spline2, fig.cap = "(ref:spline2)", cache = TRUE}
...
(ref:overlay) Overlaid spline functions fitted for t-copula with df 5 and 10.

```{r overlay, fig.cap = "(ref:overlay)"}
...
Therefore, we consider weighted smoothing spline to constrain fitted curve to be
close to $(0, 0)$ and $(1, 1)$. The $(-1, -1)$ will be approached by symmetry.
The weight on data points $(0, 0)$ and $(1, 1)$ is set to be $1,000$, which is
much larger than the weight on other data points.

(ref:wSmooth-cap) Fitted Spearman's rho by weighted smoothing spline and
symmetry.

```{r wSmooth, fig.cap = "(ref:wSmooth-cap)"}
...
If we check the fitted Spearman's rho at 0 or 1 ($-1$), we will find that the
difference between true value and fitted value is negligible.
...
(ref:cobs-cap) Fitted Spearman's rho by qualitatively constrained smoothing
spline and symmetry.

```{r cobs, message = FALSE, fig.cap = "(ref:cobs-cap)"}
library(cobs)
